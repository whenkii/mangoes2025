...
drop table shipments_details;

drop table shipments;

create table shipments(id NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY primary key,
                       name varchar2(100) not null,
                       category varchar2(100) default 'INTERNAL',
                       comments varchar2(1000) not null,
                       ts date default sysdate,
                       added_by varchar2(100));   
                       
create table shipments_details(shipment_id number,
                             name varchar2(100) not null,
                            qty number,
                            ts date default sysdate,
                            added_by varchar2(100)
                            ,constraint shipments_details_shipments_fk foreign key (shipment_id) references shipments(id)
                            ,constraint shipments_details_products_fk foreign key (name)  references products(name));
                       

create table damages(id NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY primary key,
                     shipment_id number,
                     ...
                     comments varchar2(1000) not null,
                     ts date default sysdate,
                     constraint damages_orders_fk references orders(id),
                     constraint damages_shipments_fk references shipment_id(id),
                     );
